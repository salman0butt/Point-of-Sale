      ...
      <div class="box-body">
        <div style="overflow-x:auto;">
          <table id="orderlisttable" class="table table-striped">
            ...
            <tbody>
              <?php
    $select=$pdo->prepare("select * from purchase_invoice order by invoice_id desc");
...
while($row=$select->fetch(PDO::FETCH_OBJ)  ){
    ...
    <td><span class="badge badge-primary">'.$row->payment_type.'</span></td>
       ...
       <td>
<a href="single_supplier_invoice.php?invoice_id='.$row->invoice_id.'" class="btn btn-success btn-sm" role="button" target="_blank"><i class="fas fa-eye" data-toggle="tooltip"  title="Print Invoice"></i></span></a>  
    </td>
    <td>
<a href="invoice_purchase_db.php?id='.$row->invoice_id.'" class="btn btn-success btn-sm" role="button" target="_blank"><i class="fas fa-print" data-toggle="tooltip"  title="Print Invoice"></i></span></a>  
    </td>
    
    
    <td>
<a href="edit_purchase.php?id='.$row->invoice_id.'" class="btn btn-info" role="button"><i class="fas fa-edit" data-toggle="tooltip"  title="Edit order"></i></a>   
...
$(document).ready(function() {
  $('.btndelete').click(function() {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: 'handlers/purchasedelete.php',
          type: 'post',
          data: {
            pidd: id
          },

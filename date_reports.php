...
<div class="content-wrapper">
  ...
  <div class="content">
    <div class="container-fluid">
      <form action="" method="post" name="">
        <div class="box-header with-border">
          <h3 class="box-title">Date
            <?php echo date('d',strtotime($_POST['date'])); ?> 
          </h3>
        </div>
        <!-- /.box-header -->
        <!-- form start -->
        <div class="box-body">
          <div class="row">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control pull-right" id="datepicker1" name="date" data-date-format="yyyy-mm-dd">
              </div>
            </div>
       <!--      <div class="col-md-5">
              <div class="input-group">
    ...
    $select=$pdo->prepare("select sum(total) as total , sum(subtotal) as stotal,count(invoice_id) as invoice from invoice where DAY(order_date) = :date");
      $select->bindParam(':date',date('d',strtotime($_POST['date'])));  
...
$invoice=$row->invoice;                    
          ...
          <table id="salesreporttable" class="table table-striped">
            ...
            <tbody>
              <?php
    $select=$pdo->prepare("select * from invoice where DAY(order_date) = :date");
      $select->bindParam(':date',date('d',strtotime($_POST['date'])));  
             // $select->bindParam(':todate',$_POST['date_2']);  
